...
func TestFormatWikiTimestamp(t *testing.T) {
	...
	// 1700000000 → 2023-11-14T22:13:20Z. Compare on a stable substring rather
	// than full equality so the test stays valid in any host timezone.
	got := formatWikiTimestamp("1700000000")
	if !strings.HasPrefix(got, "2023-11-14") && !strings.HasPrefix(got, "2023-11-15") {
		t.Fatalf("formatWikiTimestamp(1700000000) = %q, want a 2023-11-14/15 timestamp", got)
	}
}
...
func TestWikiNodeGetMountedExecuteParsesURLAndFormatsOutput(t *testing.T) {
	...
	if got, _ := data["updated_at"].(string); !strings.HasPrefix(got, "2023-11-1") {
		t.Fatalf("updated_at = %#v, want 2023-11-1* formatted", data["updated_at"])
	}
	if got, want := data["url"], "https://www.feishu.cn/wiki/wikcnABC"; got != want {

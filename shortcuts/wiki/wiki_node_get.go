...
// creator / updated_at / url) and is intended as the "what am I about to
...
var WikiNodeGet = common.Shortcut{
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		out := wikiNodeGetOutput(runtime, node, raw)
...
// values (title/obj_type/obj_token/parent_node_token/creator/updated_at/url)
...
// get_node.
func wikiNodeGetOutput(runtime *common.RuntimeContext, node *wikiNodeRecord, raw map[string]interface{}) map[string]interface{} {
	...
	if u := common.BuildResourceURL(runtime.Config.Brand, "wiki", node.NodeToken); u != "" {
		out["url"] = u
	}
	return out
}
...
// wiki.spaces.get_node) into an RFC3339 local-time string for the formatted
// pretty output. Returns "" when the input is empty or not numeric so the
// pretty renderer falls back to "-".
